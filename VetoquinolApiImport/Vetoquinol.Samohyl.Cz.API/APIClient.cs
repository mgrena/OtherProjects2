...
namespace Vetoquinol.Samohyl.Cz.API;
...
public class APIClient : IAPIClient
{
    ...
    private readonly string ClientCode = "VETOQUINOL";
    ...
    public async Task<ResultReport> GetProductsAsync()
    {
        ...
        IList<Product> aList = aTask.returnProductList.ToList().Select(i => new Product()
        {
        ...
        aResult.RetunObjects.Add("products", aList);
    ...
    public async Task<ResultReport> GetClientsAsync()
    {
        ...
        try
        {
            DataExchange_PortClient aClient = new();
            aClient.ClientCredentials.UserName.UserName = Account;
            aClient.ClientCredentials.UserName.Password = Password;

            int anErrorCode = 999;
            string anErrorText = "test";
            var aTask = await aClient.GetClientsAsync(new GetClients(ClientCode, new ClientList(), anErrorCode, anErrorText));
            var aTaskResult = aTask.returnClients.ClientLine;
            IList<PharmacyApi> aList = [];
            if (aTaskResult != null && aTaskResult.Length > 0)
            { 
                aList = aTask.returnClients.ClientLine.Select(i => new PharmacyApi()
                {
                    DistrId = ClientId,
                    ClientId = i.ClientId,
    ...
    public async Task<ResultReport> GetSalesAsync(DateTime startingDate, DateTime endingDate)
    {
        ...
        try
        {
            ...
            int anErrorCode = 999;
            string anErrorText = "test";
            var aTask = await aClient.GetSalesAsync(new GetSales(ClientCode, startingDate, endingDate, new SalesList(), anErrorCode, anErrorText));
            anErrorCode = aTask.returnErrorCode;
            anErrorText = aTask.returnErrorText;
            IList<SaleApi> aList = aTask.returnSalesList.SalesEntry.Select(i => new SaleApi()
            {
                DistrId = ClientId,
                ProductId = i.ProductID,
    ...
    public async Task<ResultReport> GetStockAsync()
    {
        ...
        try
        {
            ...
            int anErrorCode = 999;
            string anErrorText = "test";
            GetStock aRequest = new() { listID = ClientCode, returnStock = new(), returnErrorCode = anErrorCode, returnErrorText = anErrorText };
        
            var aTask = await aClient.GetStockAsync(aRequest);
            var aTaskResult = aTask.returnStock.StockTO;
            IList<StockApi> aList = [];
            if (aTaskResult != null && aTaskResult.Length > 0)
            { 
                aList = aTaskResult.ToList().Select(i => new StockApi()
                {
                    DistrId = ClientId,
                    ProductId = i.productId,
                    BatchNo = i.batchNo,
